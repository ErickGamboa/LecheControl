...
/// Cuánto aportó cada vaca, usando el precio real que pagó la planta.
class _RentabilidadPorVaca extends StatelessWidget {
  const _RentabilidadPorVaca({
  ...
  Widget build(BuildContext context) {
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text(
                'Anotá los litros junto al monto que pagó la planta y acá vas '
                'a ver cuánto aportó cada vaca, con el precio real de la '
                'semana.',
              ),
            ],
          ),
    ...
    return FutureBuilder<List<FilaRentabilidadSemanal>>(
      // La clave fuerza el recálculo cuando cambian los montos de la semana.
      key: ValueKey(
        '${semana.id}-${resumen.totalIngresos}-${resumen.totalGastos}',
      ),
      future: finanzasRepo.rentabilidadPorVaca(
        lecheriaId: lecheriaId,
        semana: semana,
      ),
      builder: (context, snap) {
        final filas = snap.data ?? const <FilaRentabilidadSemanal>[];
        if (filas.isEmpty) {
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  ...
                  '(${colones(resumen.precioRealPorLitro!)}/L) y los gastos '
                  'repartidos entre las ${filas.length} vacas en ordeño.',
                  style: Theme.of(context).textTheme.bodySmall,
                ),
                const SizedBox(height: 8),
                      ...
                      for (final f in filas)
                        DataRow(
                          cells: [
                            ...
                            DataCell(Text(f.litros.toStringAsFixed(1))),
                            DataCell(Text(colones(f.ingreso))),
                            DataCell(Text(colones(f.costoAsignado))),
                            DataCell(
                              Text(
                                colones(f.utilidad),
                                style: TextStyle(
                ...
                if (filas.any((f) => f.enRetiro)) ...[
                  const SizedBox(height: 6),
                  Text(
                    'Las vacas con 🚫 están en retiro: su leche se descarta, '
                    'así que no genera ingreso.',
                    style: Theme.of(context).textTheme.bodySmall,
                  ),
                ],
              ],
