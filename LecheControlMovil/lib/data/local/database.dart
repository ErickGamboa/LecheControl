...
QueryExecutor _abrirConexion() {
  ...
  return driftDatabase(
    name: nombre,
    // En web, drift necesita que le digan dónde están el módulo de SQLite
    // compilado a WebAssembly y el worker que atiende la base en segundo
    // plano; sin esto `driftDatabase` tira ArgumentError al abrir. Las rutas
    // son relativas a la raíz servida, así que los dos archivos van en la
    // carpeta `web/` del proyecto que compila a web (LecheControlWeb), y
    // tienen que ser de las versiones exactas del pubspec.lock
    // (sqlite3 3.5.0 y drift 2.34.3): un worker de otra versión revienta en
    // runtime, no al compilar.
    //
    // En Android e iOS estas opciones se ignoran por completo (la
    // implementación nativa de drift_flutter las recibe y no las mira), así
    // que declararlas acá no cambia nada en el teléfono.
    web: DriftWebOptions(
